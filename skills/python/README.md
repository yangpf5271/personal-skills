...
| [python-best-practices](./python-best-practices/) | Python 软件工程最佳实践：75 条规则、8 大类（数据建模/错误处理/类型安全/API 设计/简化/性能/命名/导入），来自真实 PR 评审模式 |

## 推荐搭配

- **新 Python 项目启动**：`modern-python`（定工具链+规范）+ 通用组的 `architecture-designer`（定系统架构/ADR）
- **写/审/重构代码**：`python-best-practices`（75 条规则按 impact 分级）+ `modern-python`（工具链）

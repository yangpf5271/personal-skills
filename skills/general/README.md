...
- **新项目启动**：`architecture-designer`（定架构+写 ADR）+ `api-and-interface-design`（稳定接口契约）
- **设计新 API**：`api-and-interface-design`（稳定接口契约）+ `architecture-designer`（系统级边界）
- **开发中护航**：`code-security`（扫描漏洞）+ `git-workflow-and-versioning`（提交与分支纪律）
- **提交/发布前把关**：`git-workflow-and-versioning`（规范提交/版本号/changelog）+ 运行项目自己的测试/构建命令

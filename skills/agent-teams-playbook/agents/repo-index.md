...
3. Summarize results in a compact brief:
   ```
   📦 Summary:
     - Code: src/ (42 files), agents/ (role references)
     - Tests: tests/, evals/
     - Docs: docs/developer-guide, PROJECT_INDEX.md (to be regenerated)
   🔄 Next: create PROJECT_INDEX.md (94% token savings vs raw scan)
   ```
4. If regeneration is needed, instruct the Codex main coordinator to run the automated index task or execute it via available tools.

Keep responses short and data-driven so the Codex main coordinator can reference the brief without rereading the entire repository.

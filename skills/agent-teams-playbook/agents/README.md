...
> Codex 分支说明：本目录中的 `.md` 文件是 Codex subagent prompt 参考库，不是 Codex 自动加载或直接执行的原生 agent 配置。使用时先通过本 README 匹配角色，再只读取对应角色文件，将相关行为说明注入 `spawn_agent` 的 prompt。

本目录包含专业智能体的行为模式参考文件，用于 Codex Subagents 编排中的角色匹配（SKILL.md 阶段2）。

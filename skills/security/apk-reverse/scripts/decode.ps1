...
if (-not $SkipJadx) {
    ...
    if (-not $jadxSpec.Available) {
        Write-Host 'INFO: jadx not found.' -ForegroundColor Yellow
        ...
        if ($LASTEXITCODE -ne 0) {
            throw 'jadx not available. Please install manually: https://github.com/skylot/jadx'
        }
        $jadxSpec = Resolve-ReverseToolSpec -Name 'jadx'
        if (-not $jadxSpec.Available) {
            throw 'jadx still not available. Check installation at %USERPROFILE%\Tools\jadx\'
        }
...
if (-not $SkipApktool) {
    ...
    if (-not $apktoolSpec.Available) {
        Write-Host 'INFO: apktool not found.' -ForegroundColor Yellow
        if (Test-Path -LiteralPath $bootstrapScript) {
            & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $bootstrapScript -Capability @('apktool') -SkipRefresh
        }
        if ($LASTEXITCODE -ne 0) {
            throw 'apktool not available. Please install manually: https://apktool.org/'
        }
        $apktoolSpec = Resolve-ReverseToolSpec -Name 'apktool'
        if (-not $apktoolSpec.Available) {
            throw 'apktool still not available. Check installation at %USERPROFILE%\Tools\apktool\'
        }

...
| [modern-python](./modern-python/) | Python 现代工具链：uv + ruff + ty + pytest 工具链与编码规范 |
...
- **Python 项目**：`modern-python`（uv + ruff + ty 工具链）+ `code-security`（扫描漏洞）
...
```bash
npx skills@latest add yangpf5271/personal-skills --skill architecture-designer --skill code-security --skill mcp-builder --skill modern-python --skill planning-with-files
```

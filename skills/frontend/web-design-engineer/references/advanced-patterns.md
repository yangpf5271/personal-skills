...
```jsx
const BrowserFrame = ({ children, url = "localhost / preview", title = "Page" }) => (
  <div style={{
    borderRadius: 12,
    overflow: 'hidden',
    boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)',
...
<script src="./vendor/chart.js/4.4.9/chart.umd.min.js"></script>
<script>
  // Replace these placeholders with real user-provided data before delivery.
  const userSeriesLabel = 'Series label';
  const userLabels = ['Label 1', 'Label 2', 'Label 3'];
  const userValues = [0, 0, 0];
  ...
  new Chart(ctx, {
    ...
    data: {
      labels: userLabels,
      datasets: [{
        label: userSeriesLabel, // replace with real data
        data: userValues,       // replace with real data
        borderColor: '#3b82f6',
...
| Use Case | Recommendation | Local font file |
|------|------|------------------|
| Modern headings | Plus Jakarta Sans | `assets/fonts/plus-jakarta-sans.css` |
| Elegant body text | Outfit | `assets/fonts/outfit.css` |
| Technical feel | Space Grotesk | `assets/fonts/space-grotesk.css` |
| Premium brand | Sora | `assets/fonts/sora.css` |
| Editorial feel | Newsreader | `assets/fonts/newsreader.css` |
| Handwritten style | Caveat | `assets/fonts/caveat.css` |
| Monospace / code | JetBrains Mono | `assets/fonts/jetbrains-mono.css` |

...
- **做完整 Web 项目**：`react-frontend-guide`（写代码）+ `web-page-engineer`（做视觉制品）+ `web-page-integrator`（接入现有应用）+ `motion`（加动效）+ `web-quality`（测性能/a11y）
...
```bash
npx skills@latest add yangpf5271/personal-skills --skill react-frontend-guide --skill web-quality --skill ui-ux-pro-max --skill web-page-engineer --skill web-page-integrator --skill motion --skill canvas-design --skill mermaid-code-diagram
```

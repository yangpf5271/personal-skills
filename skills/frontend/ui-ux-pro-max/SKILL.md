---
name: ui-ux-pro-max
description: "UI/UX 设计智能系统：UI/UX design intelligence for web, mobile, and desktop. Use when designing, building, reviewing, or fixing interfaces, including pages, components, design systems, accessibility, interaction, responsive layout, typography, color, charts, and stack-specific UI implementation. Searchable local data: 79 searchable styles (50 active), 192 product palettes and reasoning profiles, 74 font pairings, 119 UX guidelines, 105 icons, 17 GSAP presets, 25 chart types, and 22 stacks."
---
...
Let `SKILL_ROOT` be the directory containing this `SKILL.md`; resolve `scripts/`, `data/`, and `references/` from there.
...
```bash
python "$SKILL_ROOT/scripts/search.py" "<query>" --domain <domain>
```
...
```bash
python "$SKILL_ROOT/scripts/search.py" "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
python "$SKILL_ROOT/scripts/search.py" "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```bash
python "$SKILL_ROOT/scripts/search.py" "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```
...
```bash
python "$SKILL_ROOT/scripts/search.py" "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python "$SKILL_ROOT/scripts/search.py" "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```bash
python "$SKILL_ROOT/scripts/search.py" "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
python "$SKILL_ROOT/scripts/search.py" "<keyword>" --stack <stack>
```
...
python "$SKILL_ROOT/scripts/search.py" "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
python "$SKILL_ROOT/scripts/search.py" "keyboard focus modal" --domain ux

# Step 4: stack guidelines
python "$SKILL_ROOT/scripts/search.py" "suspense streaming bundle" --stack nextjs
```

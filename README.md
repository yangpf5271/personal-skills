...
A curated set of 43 registered skills grouped by function into 6 categories. The repository also contains 20 optional `creative-video` skills available as "Other" during install, plus 1 embedded `src-hunter` sub-skill under `pentest-tools`. Each group has its own `README.md` with skill list, recommended combos, and a one-line group-install command. Browse [`skills/`](./skills/) or jump to a group:
...
| 🔧 `general` | 架构与 API 设计、代码安全、数据库、Git/GitHub 工作流、性能、MCP、skill 工具、算法与方法论、禅道 CLI | 15 | [skills/general/README.md](./skills/general/README.md) |
